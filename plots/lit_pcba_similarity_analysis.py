"""
ECFP4 pairwise Tanimoto similarity analysis for the LIT-PCBA ligand set.

Mirrors dude/ecfp4_similarity_analysis.py so the two datasets are processed
with identical fingerprinting parameters (ECFP4 = Morgan radius 2, 2048 bit)
and sampling strategy, making their outputs directly comparable.

For every target in <lit-pcba-dir>/<target>/{actives.smi,inactives.smi} this
script:
  1. Randomly (stratified actives/inactives) samples up to
     --target-sample-size ligands per target and computes ECFP4 fingerprints.
  ...
  3. Pools a random subsample (--global-sample-size) drawn from all sampled
     ligands (regardless of target) -> global distribution ("all fingerprints").
  4. Writes summary CSVs, a raw-data .npz, and three plots to --output-dir.

Usage:
    python lit_pcba_similarity_analysis.py \
        --lit-pcba-dir /path/to/lit-pcba \
        --output-dir plots/output/lit-pcba/similarity_analysis
"""
import argparse
...
def main():
    ...
    parser.add_argument("--lit-pcba-dir", default=str(paths.LIT_PCBA_DIR),
                         help="Directory containing one subfolder per LIT-PCBA target.")
    parser.add_argument("--output-dir", default=str(paths.out("lit-pcba", "similarity_analysis")))
    parser.add_argument("--target-sample-size", type=int, default=300,
                         help="Max ligands sampled per target (stratified actives/inactives).")
    ...
    lit_pcba_dir = Path(args.lit_pcba_dir)
    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    target_dirs = sorted(p for p in lit_pcba_dir.iterdir() if p.is_dir()
                          and (p / "actives.smi").exists())
    print(f"Found {len(target_dirs)} LIT-PCBA targets in {lit_pcba_dir}")
    ...
    for i, target_dir in enumerate(target_dirs, 1):
        ...
        actives, inactives = get_ligand_smiles(target_dir)
        act_sample, inact_sample = sample_stratified(actives, inactives, args.target_sample_size, rng)

        fps = []
        n_act_ok = n_inact_ok = 0
        for smi in act_sample:
            fp = smiles_to_fp(smi, args.radius, args.n_bits)
            if fp is not None:
                fps.append(fp)
                n_act_ok += 1
        for smi in inact_sample:
            fp = smiles_to_fp(smi, args.radius, args.n_bits)
            if fp is not None:
                fps.append(fp)
                n_inact_ok += 1
        ...
        per_target_meta.append({
            ...
            "n_inactives_total": len(inactives),
            "n_actives_sampled": n_act_ok,
            "n_inactives_sampled": n_inact_ok,
            "n_sampled": len(fps),
        })
        print(f"[{i}/{len(target_dirs)}] {target}: sampled {len(fps)} ligands "
              f"({n_act_ok} actives, {n_inact_ok} inactives)")
    ...
    plot_two_kde_comparison(
        ...
        "Global vs. within-target ECFP4 similarity (LIT-PCBA)",
